...
(() => {
  ...
  const workdays = {
    ...
    2026: `
      ...
      08-01 08-03 08-04 08-05 08-06 08-07 08-08 08-10 08-11 08-12 08-13 08-14 08-15 08-17 08-18 08-19 08-20 08-21 08-22 08-23 08-24 08-25 08-26 08-27
    `
  };
  ...
  for (const activity of githubActivity.trim().split(/\s+/)) {
    const [key, rawCount] = activity.split(":");
    const count = Number(rawCount);
    if (!days[key]) {
      days[key] = {
        color: "var(--github-pixel)",
        label: `GitHub activity · ${count} ${count === 1 ? "contribution" : "contributions"}`,
        description: "Filled from GitHub activity history."
      };
    }
  }
